...
class ARDRegularizedLaplacianKernel(Kernel):
    r"""
    ...
    def __init__(self, fourier_freq: Iterable[Tensor], fourier_basis: Iterable[Tensor],
        ...
        self.register_parameter(name="raw_beta", parameter=torch.nn.Parameter(torch.zeros(*self.batch_shape, self.n_variables)))
    ...
    def forward(self, x1: torch.LongTensor, x2: torch.LongTensor,
        ...
        for n in range(self.n_variables):
            ...
            freq_regularizer = 1.0 / (1 + torch.narrow(self.beta, -1, n, 1) * self.fourier_freq[n])
            ...
            if diag:
                ...
                gram_factor = (basis_x.unsqueeze(-2)
                               * freq_regularizer.unsqueeze(-2).unsqueeze(-2)
                               * basis_x2.unsqueeze(-3)).sum(dim=-1)
...
if __name__ == '__main__':
    ...
    for _r in range(_n_runs):
        ...
        _kernel = ARDRegularizedLaplacianKernel(_fourier_freq, _fourier_basis)
