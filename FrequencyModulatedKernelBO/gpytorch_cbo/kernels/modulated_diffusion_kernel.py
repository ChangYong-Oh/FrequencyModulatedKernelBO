...
from FrequencyModulatedKernelBO.gpytorch_cbo.kernels.ard_diffusion_kernel import ARDDiffusionKernel


class ModulatedDiffusionKernel(ARDDiffusionKernel):
    r"""
    ...
    def __init__(self, n_modulators: int, fourier_freq: List[Tensor], fourier_basis: List[Tensor],
                 alpha_prior: Optional[Prior] = None, alpha_constraint: Optional[Interval] = Positive(),
    ...
    def forward(self, x1: torch.Tensor, x2: torch.Tensor,
        ...
        for n in range(self.n_variables):
            ...
            # freq_regularizer : b X |V| / 3 is chosen so that eigenvalues lies on
            freq_regularizer = torch.exp(-beta_n * self.fourier_freq[n] * (1 + alpha_n * modulatorls_inv_sq_mean))
            ...
            if diag:
                # alpha_n * context_dist_sq : b X (n1 = n2) / self.fourier_freq[n] : |V|
                # modulated_freq_regularizer : tensor of size : b X (n1 = n2) X |V|
                modulated_freq = torch.exp(-beta_n * self.fourier_freq[n] *
                                           (1 + alpha_n * modulator_dist_sq).unsqueeze(-1))
                gram_factor = (basis_x1_v * modulated_freq * basis_x2_v).sum(dim=-1)
            else:
                # alpha_n * context_dist_sq : b X n1 X n2 / self.fourier_freq[n] : |V|
...
if __name__ == '__main__':
    ...
    for _r in range(_n_runs):
        ...
        _kernel = ModulatedDiffusionKernel(_n_modulators, _fourier_freq, _fourier_basis, batch_shape=_batch_size)
